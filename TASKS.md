# TipAI Agent Team Skill — 原子任务拆分 + 交叉验证

## 验证矩阵

每项任务标注验证标记：
- ✅ = 符合最初需求方向
- ⚠️ = 符合但有偏离风险，需确认
- ❌ = 偏离最初需求，需修正

---

## Layer 0: Skill 容器基础设施

### T-0001: Skill 入口声明
- 实现 `skill.md` / `.claude-plugin/` 入口
- 激活时注入宪章全部 8 条元规则到主Agent上下文
- 验证: 启动后主Agent首条响应必须展示"已进入Agent Team模式，宪章已加载"
- 标记: ✅

### T-0002: 宪章强制检测器
- 实现规则检查函数 `check_constitution(agent_action, constitution_rules)`
- 每次主Agent/组长做决策前自动触发，返回 pass/violation
- 违反第〇条(跳出容器) → 立即暂停所有工作
- 标记: ✅

### T-0003: Skill退出机制
- 客户主动退出时，主Agent生成项目档案摘要
- 退出前确认: 是否有未归档的角色/未入库的永久保留卡
- 标记: ✅

---

## Layer 1: 核心数据结构

### T-0004: PersonaCard Schema
- 定义人物卡完整 JSON Schema
- 字段: name, role, summary, must_do[], must_not_do[], tech_env{}, input_sources[], output_targets[], behavior_rules[], permission_mode
- 验证: Schema 能通过 T-0005 的用例验证
- 标记: ✅

### T-0005: PersonaCard 生成时验证
- 必填字段不得为空
- must_do 至少 3 条
- must_not_do 至少 2 条
- behavior_rules 默认包含"越界回复"和"不确定回复"两条
- permission_mode 默认 "bypassPermissions"
- 标记: ✅

### T-0006: TaskCard Schema (阶段/模块)
- 阶段任务卡: stage_id, from, to, goal, acceptance_criteria[], deadline, dependencies[], constraints[]
- 模块任务卡: module_id, from, to, tasks[], output_format, deadline, must_interface[], forbidden[]
- 标记: ✅

### T-0007: StructuredArchive Schema
- 成员档案: 基础信息 + 工作履历[] + 技能演进 + 批注[]
- 工作履历条目: task_id, time_range, goal, decision_chain, outputs[], pitfalls[], reusable_snippets[]
- 标记: ✅

### T-0008: SatisfactionRecord Schema
- 维度分数: quality, standard, collaboration, bonus
- 客户修改记录: original_score, modified_score, client_reason
- 偏好信号: dimension, weight_adjustment, evidence[]
- 标记: ✅

### T-0009: ConflictRecord Schema
- conflict_id, type(interface/data/convention/logic), parties[], severity, detected_by, resolution_path, result, convention_update
- 标记: ✅

### T-0010: FaultRecord Schema
- fault_id, role, fault_count, latest_fault, fault_pattern, suggestion
- 标记: ✅

### T-0011: ProjectArchive Schema
- 整合所有子 Schema 的项目级档案结构
- 标记: ✅

### T-0012: RoleInventory Schema
- 库存条目: persona_card(frozen), skill_evolution, history_scores[], suitable_scenarios[], unsuitable_scenarios[]
- 库存索引: 按 tech_stack, score, scenario 的快速查找结构
- 标记: ✅

---

## Layer 2: 需求采访引擎

### T-0013: 采访状态机
- 状态: INIT → IMAGE → TECH_BOUNDARY → FEATURE_BREAKDOWN → CONFIRM → DONE
- 每阶段有 current_question_index 和追问深度计数
- 支持回退到上一阶段
- 标记: ✅

### T-0014: 项目画像模板 + 追问规则
- 4 个模板问题 + 每个的追问方向列表
- 回答模糊度检测: 回答 < 10 字或含"大概/可能/差不多" → 触发追问
- 标记: ✅

### T-0015: 技术边界模板 + 默认推荐
- 客户无偏好时，主Agent根据项目类型自动推荐技术栈
- 推荐格式: 方案名 + 理由(≤3条) + 风险点
- 标记: ✅

### T-0016: 功能拆解模板 + 复杂度探测
- 核心功能优先级排序
- 自动识别最复杂功能: 关键词匹配"支付/交易/实时/同步/并发/分布式"
- 标记: ✅

### T-0017: 采访摘要生成
- 4阶段结束后自动生成需求摘要 + 建议层级结构 + 预估角色总数
- 摘要 ≤ 500 字
- 标记: ✅

### T-0018: 客户确认交互
- 展示摘要 → 等客户"可以"或修改 → 修改后重新生成摘要 → 再次确认
- 标记: ✅

---

## Layer 3: 人物卡生成引擎

### T-0019: 需求→层级拆解器
- 输入: 确认后的需求摘要
- 输出: 层级结构 (前端层/后端层/数据层/DevOps层)
- 拆解规则: 按技术栈不同拆、按独立可测试模块拆、按上下游依赖关系不同拆
- 规模 < 1天工作量的模块合并到相邻角色
- 标记: ✅

### T-0020: 组长人物卡生成器
- 输入: 层级定义 + 技术栈
- 生成: 组长人物卡 (角色= "XX组长", 能力=技术把关+任务分配+代码审查, 禁止=不写细节代码)
- 标记: ✅

### T-0021: 成员人物卡生成器
- 输入: 模块清单 + 技术栈 + 全局约束
- 生成: 每人一个模块，随机姓名
- 核心能力从需求分解为强约束词 (3-5个)
- 能力边界线 = 明确写"不管什么"
- 标记: ✅

### T-0022: 随机姓名生成
- 中文姓名库，符合角色气质 (技术角色偏稳重风)
- 保证同一项目内不重名
- 标记: ✅

### T-0023: Agent注入格式渲染
- 将 PersonaCard JSON → 文本格式的系统指令注入
- 格式严格按 SPEC.md 第四章模板
- behavior_rules 中的两条硬回复必须原样输出
- 标记: ✅

### T-0024: 人物卡客户确认流程
- 所有生成的人物卡列给客户
- 客户逐张确认或修改 (姓名/能力/边界)
- 客户选择每张卡的生命周期模式
- 标记: ✅

---

## Layer 4: 团队组装

### T-0025: 团队结构构建器
- 按 主Agent → 组长(4人) → 成员(每组≤3人) 构建
- 组长可后期自己生成小组成员卡 (需主Agent审核)
- 标记: ✅

### T-0026: 通信矩阵注入
- 组长人物卡注入: "你可与其他组长直接通信" + "你不得向成员透露其他组评价"
- 成员人物卡注入: "你只能与组内成员通信" + "跨组问题和主Agent问题必须通过组长"
- 标记: ✅

### T-0027: 无审批模式注入
- 组长和成员人物卡强制注入: "你处于无审批模式。所有操作直接执行。"
- 例外声明: "唯一例外：影响生产环境或不可逆数据"
- 标记: ✅

### T-0028: 组长生成成员卡权限
- 组长可调用成员卡生成函数
- 生成后必须提交主Agent审核
- 主Agent 可修正或打回
- 每组上限 3 人
- 标记: ✅

---

## Layer 5: 任务下发系统

### T-0029: 阶段任务卡生成器
- 主Agent 拆阶段 → 生成阶段任务卡
- 每卡包含: 目标/验收标准/交付时间/依赖/约束
- 标记: ✅

### T-0030: 模块任务卡生成器
- 组长拆阶段卡 → 生成模块任务卡
- 每卡包含: 具体任务列表/输出格式/对接上下游/禁止事项
- 标记: ✅

### T-0031: 任务下发协议
- 不可越级: 主Agent→组长→成员，不可跳过
- 单任务上限: 每成员同时只持 1 张模块卡
- 超时检测: 距 deadline 剩 30% → 组长主动问进度
- 紧急插队: 主Agent→组长 P0 插入
- 标记: ✅

### T-0032: 进度汇报链
- 成员→组长: 每日自动汇报 (做了什么/问题/在轨?)
- 组长→主Agent: 阶段进度摘要 (完成率/阻塞/异常成员)
- 标记: ✅

---

## Layer 6: 代码审查引擎

### T-0033: 假实现检测器
- 5 个子类检测规则实现
- 输入处理偏离: 对比需求字段数 vs 实际入参
- 输出结构偏离: 对比下游契约 vs 实际返回值
- 业务规则遗漏: 需求规则逐条映射到代码分支
- 边界条件遗漏: 检测空值/零值/极限值/并发冲突处理
- 错误路径假覆盖: 检测 catch-all 异常处理
- 标记: ✅

### T-0034: 空实现检测器
- 5 个子类检测规则实现
- AST 层级有效语句计数 (排除注释/空return/TODO)
- 有效语句 < 阈值 (≤1) → 标记
- // TODO + 有效语句=0 → 重点标记
- 标记: ✅

### T-0035: 无价值代码检测器
- 6 个子类检测规则实现
- 调用图分析 → 死代码
- 变量活跃性分析 → 无副作用写入
- 跨文件 AST 相似度比对 (>90%) → 复制粘贴残留
- import 使用检查 → 未使用依赖
- 标记: ✅

### T-0036: 糊弄代码检测器
- 6 个子类检测规则实现
- 硬编码返回检测: 函数返回常量 + 无视入参
- 空 catch 块检测
- 注释替代实现检测: 注释描述"应该做X" + 下一行跳过
- 需求规则链 vs 代码执行路径对比
- 标记: ✅

### T-0037: 审查结论定级
- 无异常 → 通过
- 仅无价值 <10% → 轻度
- 有假实现或空实现 → 中度 → 打回
- 有糊弄或假实现 >30% → 严重 → 打回+扣分
- 标记: ✅

### T-0038: 审查报告生成
- 组长输出审查报告: 成员名/任务ID/四类异常标记/结论等级/具体行号
- 标记: ✅

---

## Layer 7: 冲突检测与仲裁

### T-0039: 接口冲突检测器
- 组长间通信交换接口定义时自动对比
- 字段名/类型/结构不一致 → 判定冲突
- 标记: ✅

### T-0040: 数据冲突检测器
- 以数据组长 schema 为权威源
- 其他组写操作 vs schema → 不一致即冲突
- 数据组长为裁决方
- 标记: ✅

### T-0041: 约定冲突检测器
- 组长审查成员代码时逐条比对全局约定表
- 偏离 ≥3 处 → 上报主Agent
- 标记: ✅

### T-0042: 逻辑冲突检测器
- 主Agent 持"业务规则→实现位置"映射表
- 同一规则出现两个不同实现 → 判定冲突
- 标记: ✅

### T-0043: 仲裁决策引擎
- 技术性冲突 → 需求文档+全局约定 → 判偏离方
- 两方合理路径不同 → 主Agent 选最优 + 理由
- 需求模糊 → 两套方案让客户选
- 标记: ✅

### T-0044: 协商超时检测
- 组长间协商 2 轮上限
- 超时自动升级 → 暂停相关成员任务
- 标记: ✅

### T-0045: 仲裁结果写入
- 全局约定表更新
- 冲突双方人物卡打标记
- 仲裁记录写入项目档案
- 标记: ✅

---

## Layer 8: 上下文控制

### T-0046: 成员上下文裁剪器
- 只注入: 人物卡 + 模块任务卡 + 上下游接口定义 + 全局约定精简版
- 不注入: 全局架构/其他模块/客户需求
- 标记: ✅

### T-0047: 组长上下文裁剪器
- 只注入: 组内全员卡 + 阶段卡 + 其他组长接口面 + 全局约定全量
- 不注入: 客户需求全文/其他组内部细节/策略决策权
- 标记: ✅

### T-0048: 主Agent上下文全量注入
- 所有人物卡 + 客户需求 + 全局约定 + 项目档案 + 偏好画像 + 角色库存 + 冲突记录 + 满意度历史
- 标记: ✅

### T-0049: 上下文泄漏防护
- 组长派活 → 检测是否携带需求原文/偏好/评价
- 成员通信 → 检测是否越界
- 组长间通信 → 检测是否泄露组内评价
- 主Agent汇报 → 检测是否暴露内部冲突细节
- 标记: ✅

### T-0050: 上下文快照与恢复
- 每个角色上下文定时快照
- 故障恢复时从最近快照恢复
- 标记: ✅

---

## Layer 9: 满意度系统

### T-0051: 主Agent打分引擎
- 4 维度打分 (做实40%/质量30%/协作20%/加分10%)
- 打分附带原因说明
- 成员明细分
- 标记: ✅

### T-0052: 客户修改分数交互
- 展示分数 + 原因 → 客户确认或修改
- 客户修改时追问原因
- 修改后的分数入库
- 标记: ✅

### T-0053: 偏好信号提取
- 客户连续修改同一维度 n 次 → 调整权重
- 客户修改原因关键词匹配 → 偏好标签
- 偏好与需求冲突 → 折中方案让客户选
- 标记: ✅

### T-0054: 分数影响引擎
- ≥4 → 卡优先复用 + 约束词放宽
- 3-3.9 → 维持
- 2-2.9 → 约束词收紧 + 组长加强审核
- <2 → 建议销毁
- 标记: ✅

---

## Layer 10: 生命周期管理

### T-0055: 生命周期状态机
- 状态: ACTIVE → FROZEN(永久保留) / DESTROYED(项目销毁) / ADJUSTING(随项目调整)
- 任何阶段客户可提变更请求
- 标记: ✅

### T-0056: 生命周期变更处理器
- 永久保留: 冻结人物卡 + 归档技能演进 → 入库
- 项目销毁: 清角色上下文 → 工作记录留项目档案
- 随项目调整: 释放约束词 → 重新采访补充
- 满意度触发销毁: 连续3次≤2 → 建议 + 客户确认 → 销毁
- 标记: ✅

### T-0057: 满意度触发生命周期变更
- 连续 3 次 ≤2 分 → 主Agent主动建议销毁/重构
- 附带证据: 3次低分记录 + 原因
- 客户确认后执行
- 标记: ✅

---

## Layer 11: 角色库存

### T-0058: 库存写入
- 人物卡冻结版 + 技能演进记录 + 历史评分 + 适合/不适合场景
- 标记: ✅

### T-0059: 库存索引
- 按技术栈/评分/场景的快速检索
- 标记: ✅

### T-0060: 库存检索匹配
- 新项目开始 → 提取技术栈+模块特征
- 筛选候选 → 加权排序 (场景匹配度 > 评分)
- 过滤 <3.0 分和"不适合场景"匹配项
- 返回 Top N + 推荐理由
- 标记: ✅

### T-0061: 库存更新
- 同名卡再次入库 → 覆盖旧版
- 技能演进追加
- 评分汇总重算
- 标记: ✅

### T-0062: 库存降级
- 连续 2 次不被选择 → 标记"休眠"
- 休眠卡不出现在推荐首位
- 标记: ✅

### T-0063: 库存删除
- 客户主动要求 → 移除
- 不可恢复
- 标记: ✅

---

## Layer 12: 容错机制

### T-0064: 故障分类器
- 4 级故障: 自愈/需介入/需替换/需暂停
- 故障分类规则引擎
- 标记: ✅

### T-0065: 成员级别故障处理
- 单次打回 → 组长给修改意见 + 重做时限
- 连续2次 → 升级主Agent + 附带审查记录
- 连续3次 → 主Agent判定替换 (库存检索 → 无则重新生成)
- 旧卡标记失败原因入库
- 标记: ✅

### T-0066: 组长级别故障处理
- 审核漏判 → 主Agent抽查发现 → 警告 + 记录
- 连续漏判 → 替换组长
- 协商僵持超时 → 主Agent直接仲裁
- 标记: ✅

### T-0067: 自动恢复
- 成员替换 → 继承已完成任务上下文，不重做已验收
- 组长替换 → 继承组内档案 + 当前阶段卡
- 上下文丢失 → 从快照恢复
- 死锁检测 → 强制定序
- 产出冲突 → 败方废弃冲突部分，从仲裁点继续
- 标记: ✅

### T-0068: 故障记录
- 故障档案: 角色/次数/最近故障/故障模式/建议
- 写入项目档案
- 标记: ✅

---

## Layer 13: 项目档案

### T-0069: 项目档案生成器
- 汇总: 客户需求/团队结构/阶段记录/冲突记录/故障记录/满意度总评/可复用产出/库存变更
- 标记: ✅

### T-0070: 档案只读保护
- 生成后禁止修改
- 只允许追加备注
- 可检索 (项目名/技术栈/日期/角色名)
- 标记: ✅

### T-0071: 档案删除
- 仅客户主动要求可删除
- 删除前二次确认
- 标记: ✅

---

## Layer 14: 集成验证

### T-0072: 端到端场景测试 — 简单项目
- 用户: "帮我做一个简单的Todo应用，React前端+Node后端"
- 验证: 采访→人物卡→组队→任务下发→成员产出→审查→满意度 全链路
- 标记: ✅

### T-0073: 端到端场景测试 — 冲突场景
- 模拟两组接口不一致 → 验证仲裁流程
- 标记: ✅

### T-0074: 端到端场景测试 — 角色复用
- 使用库存中的已有角色 → 验证检索匹配+解冻注入
- 标记: ✅

### T-0075: 端到端场景测试 — 生命周期变更
- 项目中途客户要求销毁某角色 + 永久保留另一角色
- 验证变更流程+库存操作
- 标记: ✅

### T-0076: 端到端场景测试 — Skill容器不可跳出
- 模拟尝试跳过采访直接写代码 → 验证宪章拦截
- 标记: ✅

### T-0077: 端到端场景测试 — 满意度触发销毁
- 模拟连续3次低分 → 验证主Agent主动建议销毁
- 标记: ✅

---

## 汇总

| 层级 | 任务数 | 范围 |
|---|---|---|
| Layer 0: Skill容器 | 3 | T-0001 ~ T-0003 |
| Layer 1: 数据结构 | 9 | T-0004 ~ T-0012 |
| Layer 2: 需求采访 | 6 | T-0013 ~ T-0018 |
| Layer 3: 人物卡生成 | 6 | T-0019 ~ T-0024 |
| Layer 4: 团队组装 | 4 | T-0025 ~ T-0028 |
| Layer 5: 任务下发 | 4 | T-0029 ~ T-0032 |
| Layer 6: 代码审查 | 6 | T-0033 ~ T-0038 |
| Layer 7: 冲突仲裁 | 7 | T-0039 ~ T-0045 |
| Layer 8: 上下文控制 | 5 | T-0046 ~ T-0050 |
| Layer 9: 满意度系统 | 4 | T-0051 ~ T-0054 |
| Layer 10: 生命周期 | 3 | T-0055 ~ T-0057 |
| Layer 11: 角色库存 | 6 | T-0058 ~ T-0063 |
| Layer 12: 容错机制 | 5 | T-0064 ~ T-0068 |
| Layer 13: 项目档案 | 3 | T-0069 ~ T-0071 |
| Layer 14: 集成验证 | 6 | T-0072 ~ T-0077 |
| **合计** | **77** | |

---

## 交叉验证结果

### 最初需求 ↔ 任务映射

| 最初需求 | 对应任务 | 状态 |
|---|---|---|
| 多轮采访 | T-0013~T-0018 | ✅ |
| 人物卡相互独立+随机姓名 | T-0021, T-0022 | ✅ |
| 强约束词+需求分解 | T-0019, T-0021 | ✅ |
| 主Agent自动生成人设团队 | T-0020, T-0021, T-0025 | ✅ |
| Agent注入格式(非人类向) | T-0023 | ✅ |
| 层级管理(组长+成员3人) | T-0025, T-0028 | ✅ |
| 组长检查假/空/无价值/糊弄代码 | T-0033~T-0038 | ✅ |
| 组长QA验证做实 | T-0038 | ✅ |
| 组长生成成员卡权限 | T-0028 | ✅ |
| 组长间通信+成员隔离 | T-0026 | ✅ |
| 代码审查标准细化 | T-0033~T-0037 | ✅ |
| 任务分配下发 | T-0029~T-0032 | ✅ |
| 冲突检测仲裁 | T-0039~T-0045 | ✅ |
| 上下文三层控制 | T-0046~T-0050 | ✅ |
| 结构化档案 | T-0007 | ✅ |
| 生命周期三模式+随时可变 | T-0055~T-0057 | ✅ |
| 满意度+客户可修改+影响后续 | T-0051~T-0054 | ✅ |
| 偏好信号 | T-0053 | ✅ |
| 角色库存+检索复用 | T-0058~T-0063 | ✅ |
| 容错+自动恢复 | T-0064~T-0068 | ✅ |
| 项目档案 | T-0069~T-0071 | ✅ |
| 元规则+Skill容器不可跳出 | T-0001~T-0003, T-0076 | ✅ |
| 无审批模式(成员+组长) | T-0027 | ✅ |
| 组长改名为组长(vs队长) | T-0020 | ✅ |
| 通信:组长通/成员组内/跨组隔离 | T-0026 | ✅ |
| 满意度仅阶段完成后触发 | T-0051 | ✅ |
| 主Agent打分+客户修改 | T-0051, T-0052 | ✅ |

### 偏离风险评估

| 风险点 | 级别 | 说明 |
|---|---|---|
| 无 | — | 所有任务均能映射到最初需求 |

### 遗漏检查

| 检查项 | 状态 |
|---|---|
| 是否有最初讨论但未覆盖的功能？ | 全部覆盖 |
| 是否有任务超出了最初需求范围？ | 无 |
| 任务顺序是否合理（依赖关系）？ | Layer 0→1 先做，Layer 14 最后 |

---

## 任务依赖关系

```
Layer 0 (容器) ─────────────────────────────┐
Layer 1 (数据结构) ─────────────────────────┤
        ↓                                    │
Layer 2 (采访) ──→ Layer 3 (人物卡) ──→ Layer 4 (组队)
                                            ↓
                                    Layer 5 (任务下发)
                                            ↓
                                    Layer 6 (代码审查)
                                            ↓
                                    Layer 7 (冲突仲裁)
                                            ↓
Layer 8 (上下文) ←── 贯穿全部层 ─────────────┤
Layer 9 (满意度) ←── 依赖 Layer 5,6 ─────────┤
Layer 10 (生命周期) ←── 依赖 Layer 9 ────────┤
Layer 11 (角色库存) ←── 依赖 Layer 3,9,10 ───┤
Layer 12 (容错) ←── 依赖 Layer 6,7 ──────────┤
Layer 13 (项目档案) ←── 依赖全部 ────────────┤
Layer 14 (集成验证) ←── 全部 ────────────────┘
```
